...
class SentenceAttention(nn.Module):
    ...
    def forward(self, documents, sentences_per_document, words_per_sentence):
        ...
        print(f"packed_sentences.data.size: {packed_sentences.data.size()}")
        ...
        print(f"packed_words_per_sentence.data.size: {packed_words_per_sentence.data.size()}")
        ...
        print(f"sentences.size: {sentences.size()}")
        print(f"word_alphas.size: {word_alphas.size()}")
...
def main():
    ...
    model = HierarchialAttentionNetwork(n_classes=2, vocab_size=100, emb_size=384, word_rnn_size=50, sentence_rnn_size=50,
                                        ...
                                        dropout=0.5)
    # Define some data
    documents = torch.randint(0, 100, (10, 5, 10))  # (n_documents, sent_pad_len, word_pad_len)
    sentences_per_document = torch.randint(1, 6, (10,))  # (n_documents)
    words_per_sentence = torch.randint(1, 11, (10, 5))  # (n_documents, sent_pad_len)
    # Forward pass
    scores, word_alphas, sentence_alphas = model(documents, sentences_per_document, words_per_sentence)
    print(scores.size())  # torch.Size([10, 2])
    print(word_alphas.size())  # torch.Size([10, 5, 10])
    print(sentence_alphas.size())  # torch.Size([10, 5])
